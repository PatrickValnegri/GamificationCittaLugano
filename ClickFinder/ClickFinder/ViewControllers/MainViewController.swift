            ...
            for child in snapshot.children {
                if let childSnapshot = child as? DataSnapshot{
                    ...
                    if mac == beaconID && switch_hdd == "1"{
                        self.sendNotification(titolo: "Ritrovamento mio beacon", mac: iphoneID!, beacon_id: beaconID, gps: "\(self.currentLocation.latitude)_\(self.currentLocation.longitude)")
                    }
                }
            }
        })
